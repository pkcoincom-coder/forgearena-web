...
serve(async(req)=>{
  ...
  try{
    ...
    if(!SUPABASE_URL||!ANON_KEY)throw new Error("Missing ANON_KEY");
    ...
    if(userErr)throw new Error("DB error: "+userErr.message);
    ...
    const resendRes=await fetch("https://api.resend.com/emails",{method:"POST",headers:{"Authorization":`Bearer ${RESEND_API_KEY}`,"Content-Type":"application/json"},body:JSON.stringify({from:fromAddress,to:[normalizedEmail],subject:`${code} is your Forge Arena login code`,html:`<div style="background:#0d0d0e;padding:40px;font-family:Arial;max-width:480px;margin:auto"><div style="text-align:center;margin-bottom:20px"><div style="font-size:28px;font-weight:900;color:#fff;letter-spacing:4px">FORGE</div><div style="font-size:20px;font-weight:900;color:#FFD700;letter-spacing:6px">ARENA</div></div><p style="color:#aaa;text-align:center">Your login code:</p><div style="background:#111;border:2px solid #FF6600;padding:24px;text-align:center;margin:20px 0"><span style="font-size:42px;font-weight:900;color:#FFD700;letter-spacing:12px;font-family:monospace">${code}</span><p style="color:#888;font-size:12px;margin-top:10px">Expires in 10 minutes</p></div><p style="color:#888;font-size:12px;text-align:center">If you did not request this code please ignore this email.</p><p style="color:#666;font-size:11px;text-align:center;margin-top:20px">Forge Arena · forgearena.net</p></div>`})});
